...
VKeyboard.prototype = {
    ...
    _setup_style: function(obj, top, left, width, height, position, text_align, line_height, font_size, font_weight, padding_left, padding_right)
    {
        ...
        if (padding_left)
            os.paddingLeft = padding_left;
        if (padding_right)
            os.paddingRight = padding_right;
    },
            ...
            do_gap, start_layout_index)
    {
        ...
        this._setup_event(kb_main, "selectstart", function(event) {
            return false;
        });
        this._setup_event(kb_main, "mousedown", function(event) {
        ...
        if (!exists || changed)
        {
            ...
            var kb_lang = this._setup_key(kb_main, "___lang", (prevY + prevH + gap) + "px", gap + 80 +"px", vcell, cp, l, lh, fs, n, pad);
//            kb_lang.innerHTML = "Accept";
            this.mod[7] = kb_lang;
            ...
            var kb_space = this._setup_key(kb_main, "___space", prevY, (this._findX(kb_res_2) + this._findW(kb_res_2) + gap) - 65 + "px", (6.28 * cell) + "px", cp, c, lh, fs);
...
//        edge_3 = (this._findX(kb_pad_star) + this._findW(kb_pad_star) + gap)- 50 + "px";
        ...
        if (key)
        {
            ...
            if (ks)
            {
                if (textcolor)
                    ks.color = textcolor;
                if (bordercolor)
                    ks.border = "1px solid " + bordercolor;
                if (bgcolor)
                    ks.backgroundColor = bgcolor;
                ks.borderRadius = "5px";
            }
    ...
    _refresh_layout: function(layout)
    {
        ...
        while (--i >= 0)
        {
            var key = this.keys[i], key_val = arr_cur[i];
            if (!key_val)
                key_val = "";
    ...
    _generic_callback_proc: function(event)
    {
        ...
        if (in_el.dead)
        {
            ...
            {
                val = "";
                vkb.DeadAction[0] = vkb.DeadAction[1] = null;
            }
            else
            {
                vkb.DeadAction[0] = val;
                vkb.DeadAction[1] = in_el.dead;
            }
